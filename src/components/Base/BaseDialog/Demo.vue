...
import DialogCmp from "./Dialog.vue";
import DialogCmp2 from "./Dialog.vue";
import DialogCmp3 from "./Dialog.vue";
...
const showTimes = () => {
  ...
  const curTime = `${year}年${month}月${day}日 ${hours}:${minutes}:${seconds}`;
  alert(`${curTime}`);
};
const handleConfirm = () => {
  ElMessage({
